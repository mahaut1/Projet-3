...
/* const sessionManager = () => {
    return{
        // Est ce que l'admin est authentifié? 
        refreshAdmin: (data = null)=> sessionManager().
        isAuthenticated() ? createAdmin(data=null) : deleteAdmin(),
        // si non on enlève le token du sessionstorage et on en enregistre un nouveau
          unvalidate : () => sessionStorage.removeItem("token"),
          validate:(token, expire=null)=> sessionStorage.setItem("token", token),
        // on retourne le token enregistré
        getToken: () => {
...
/* .then(data=> {
    ...
    if (newUser.userId==1){
       ...
       let errorMsg=document.getElementById('error-message')
       errorMsg.textContent="Identifiant ou mot passe incorrect !"
   }
}) 
.catch(error=>{
   console.log(error)
